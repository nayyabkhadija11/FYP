...
class CampaignsListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFFF8F9FA),
      appBar: AppBar(
        title: const Text('My Campaigns', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
        elevation: 0.5,
        centerTitle: true,
      ),
      // NEW: pehle yeh screen ek hardcoded 'teamNumber' field se filter
      // karti thi jo campaign doc main kabhi save hi nahi hota tha —
      // isliye kabhi koi campaign show nahi hota tha. Ab seedha logged-in
      // vaccinator ke UID se filter karte hain (campaign doc ke
      // 'vaccinatorIds' array main).
  ...
  Widget _buildCampaignCard(
    ...
    // NEW: 'targetAreas' (list) se area label banate hain — pehle
    // 'targetArea'/'targetVillage' fields parhe ja rahe thay jo campaign
    // doc main save hi nahi hote.
    final areasList = List<String>.from(data['targetAreas'] ?? []);
    String area = areasList.isNotEmpty ? areasList.join(', ') : (data['area'] ?? 'N/A');

    // NEW: 'teamName' field se team label — pehle 'teamNumber' parha ja
    // raha tha jo kabhi save nahi hota.
    ...
    // Progress ab is vaccinator ke apne assignments se nikalte hain
    // (campaignId + vaccinatorId dono se filter), taake progress bhi
    // sirf iski apni team ka dikhe, poori campaign ka nahi.
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          .collection('campaign_assignments')
          .where('campaignId', isEqualTo: campaignId)
