...
class _DashboardHomeContentState extends State<DashboardHomeContent> {
  ...
  // NEW: pehle yeh 'assignedVaccinators' field dhoond raha tha, jo
  // campaign doc main kabhi save hi nahi hoti — CampaignModel.toMap()
  // 'vaccinatorIds' field save karta hai. Isi mismatch ki wajah se
  // popup kabhi trigger hi nahi hota tha.
  void _listenForActiveCampaigns() {
    if (_currentUser == null) return;
    ...
    _campaignSubscription = FirebaseFirestore.instance
        .collection('campaigns')
        .where('status', isEqualTo: 'active')
        .where('vaccinatorIds', arrayContains: _currentUser?.uid)
        .snapshots()
        .listen((snapshot) {
      if (snapshot.docs.isNotEmpty) {
        var doc = snapshot.docs.first;
        ...
        if (_lastNotifiedCampaignId != campaignId && mounted) {
          _lastNotifiedCampaignId = campaignId;
          _showCampaignPopUp(
            title: (data['name'] ?? 'National Polio Campaign').toString(),
  ...
  // NEW: campaign doc main sirf 'startDate'/'endDate' Timestamps hoti
  // hain (koi seedha 'dateRange' string field nahi) — is se readable
  // string banate hain.
  String _formatDateRange(dynamic startVal, dynamic endVal) {
  ...
  bool _matchesCurrentVaccinator(Map<String, dynamic> data) {
    ...
    final vaccinatorIds = data['vaccinatorIds'];
    if (vaccinatorIds is List) {
      for (final value in vaccinatorIds) {
        if (value != null && value.toString().trim() == currentUid.trim()) {
          return true;
        }
      }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // --- CAMPAIGN BANNER FILTERED FOR ASSIGNED VACCINATOR ---
                  // NEW: 'assignedVaccinators' ki jagah 'vaccinatorIds'
                  // (CampaignModel field) — aur tap karne par seedha
                  // CampaignChildListScreen khulti hai (apne assigned
                  // bachon ki list dekhne ke liye).
                  StreamBuilder<QuerySnapshot>(
                    stream: FirebaseFirestore.instance
                        .collection('campaigns')
                        .where('status', isEqualTo: 'active')
                    ...
                    builder: (context, snapshot) {
                      ...
                      var campaignDoc = snapshot.data!.docs.first;
                      var campaignData = campaignDoc.data() as Map<String, dynamic>;
                      String campaignId = campaignDoc.id;
                      String title = (campaignData['name'] ?? 'National Polio Campaign').toString();
                      String dates = _formatDateRange(campaignData['startDate'], campaignData['endDate']);
                      List<String> targetAreas = List<String>.from(campaignData['targetAreas'] ?? []);
                      String area = targetAreas.isNotEmpty ? targetAreas.join(', ') : 'N/A';
                      String team = (campaignData['teamName'] as String?)?.isNotEmpty == true
                          ? campaignData['teamName']
                          : 'Team';
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Text('Area: ', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
                                  Expanded(child: Text(area, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
                                  const SizedBox(width: 16),
                                  Text('Team: ', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
                                  Text(team, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                                ],
