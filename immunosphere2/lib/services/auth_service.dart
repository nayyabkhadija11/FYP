...
class AuthService {
  ...
  Future<String?> registerUser({
    ...
    try {
      ...
      // NEW: valid_employees se employee ka health center yahan store hoga
      // taake supervisor sirf apne health center ke vaccinators dekh sake.
      String? healthCenter;
      ...
      if (role == 'Vaccinator' || role == 'Supervisor') {
        ...
        // NEW: is employee ID ke sath jo health center linked hai, usay
        // capture kar lein taake user doc ke sath save ho sake.
        healthCenter = (empData['healthCenter'] ?? '').toString().trim();
      }
      ...
      if (user != null) {
        ...
        Map<String, dynamic> userData = {
          ...
          'name': fullName.trim(), // UI standard display field
          'email': email.trim(),
          // NEW: role hamesha lowercase save karte hain taake queries
          // (jaise getVaccinators main .where('role', isEqualTo: 'vaccinator'))
          // hamesha consistently match karein, chahe UI main role 'Vaccinator'
          // (capital) form main kyun na pass ho.
          'phone': phone.trim(),
        ...
        // NEW: Vaccinator/Supervisor ka health center bhi save karein —
        // isi se AssignVaccinatorsScreen supervisor ke apne health center
        // ke vaccinators filter kar sakega.
        if (healthCenter != null && healthCenter.isNotEmpty) {
          userData['healthCenter'] = healthCenter;
        }
